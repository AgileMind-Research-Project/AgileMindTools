...
def create_database_connection(schema=None):
    """
    Create a database connection using credentials from environment variables.
    
    Args:
        schema (str, optional): Database schema/name. If not provided, uses DB_NAME from .env
    ...
    try:
        ...
        db_name = schema if schema else os.getenv('DB_NAME')
