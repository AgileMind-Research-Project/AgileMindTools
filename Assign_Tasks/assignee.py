...
def get_unassigned_parent_tasks(project_id, tenant):
    """
    ...
    try:
        ...
        check_query = """
            ...
            AND TABLE_NAME = 'project_backlog_priority'
        """
        check_params = {'schema': tenant}
        check_df = read_from_mysql_with_params(check_query, check_params, tenant)
        
        if check_df.empty:
            logging.warning(f"Table 'project_backlog_priority' not found in {tenant}. Skipping project {project_id}.")
            return []

        query = """
            SELECT pbp.backlog_id
        ...
        logging.info(f"Skipping project {project_id}: {str(e)}")
        return []
...
def get_developer_work_history(project_id, tenant_table, tenant_db):
    ...
    try:
        ...
        params = {'project_id': project_id}
        df = read_from_mysql_with_params(query, params, tenant_table)
...
def assign_tasks_to_developers(project_id, tenant_table, tenant_db):
    ...
    try:
        ...
        parent_task_ids = get_unassigned_parent_tasks(project_id, tenant_table)
        ...
        # Step 2: Get subtasks
        subtasks = get_subtasks_by_parent_ids(parent_task_ids, tenant_table)
        if not subtasks:
